#!/bin/bash
##SUDO
alias virsh="sudo virsh"
alias virt-viewer="sudo virt-viewer"
alias wgi="sudo wg-quick"
# alias chroot="sudo schroot"
alias std="sudo /env/bin/sdt"
alias mount="sudo mount"
alias umount="sudo umount"
alias nmap="sudo nmap"
alias hotspot="sudo hotspot"
alias bandwhich="sudo bandwhich"
alias ufw="sudo ufw"
alias btrfs="sudo btrfs"
alias ser="sudo /env/bin/ser"
alias rustdesk="sudo rustdesk"
alias in="sudo innernet"
# alias ssh="TERM=tmux-256color ssh -X"
alias ssh="waypipe ssh -R 2489:127.0.0.1:2489"
alias bolt="boltctl"

alias dot='/usr/bin/git --git-dir=$HOME/.dot/ --work-tree=$HOME'
alias run.sh="./run.sh"
alias run="./run.sh"
alias pk="sudo /env/bin/pik -o false"

##WM
alias sx="startx"
alias sw="startw"


##DEV
alias box='devbox'
alias dev='devenv'


##PACKAGE_MANAGER
alias pack='paru'
alias aly='nostromo'


##NETWORK
alias dnsinuse='nmcli dev show | grep IP4.DNS'
alias findip='sudo arp-scan --localnet'
alias netspeed='wget --output-document=/dev/null http://speedtest.wdc01.softlayer.com/downloads/test500.zip'
alias portmon='sudo lsof -nP -iTCP -sTCP:LISTEN'
alias openports='sudo netstat -tulpn'
alias zero='sudo zerotier-cli'
alias ssm='manssh'
alias blue='bluetuith'
alias ip="ip -c"
alias ipi="ipinfo myip"

##SYSTEM
alias moonlight="QT_QPA_PLATFORM=xcb moonlight"
alias hibernate="systemctl hibernate"
alias cp="rsync -uahP --inplace --no-whole-file --info=progress2"
alias cat='bat --paging=never'
alias ll='eza -liSHF --header --git --group-directories-first --git-ignore'
alias la='eza -laiSHF --header --git --group-directories-first --git-ignore'
alias lg='eza -laiSHF --header --git --group-directories-first'
alias lt='eza -liSHF --header --git --git-ignore --group-directories-first --tree -L5'
alias lu='dutree -d1 --usage -x .git -x .direnv'
alias df='dfc -dTmw -p -/dev/loop,tmp,dev,run,one,efivarfs'
alias lsblk='lsblk --output NAME,SIZE,FSUSE%,MODEL,TRAN,TYPE,FSTYPE,UUID,LABEL,MOUNTPOINT,STATE,HOTPLUG'
alias lsmnt='lsblk --output LABEL,NAME,SIZE,FSTYPE,MOUNTPOINTS'
alias vols='sudo lvm pvs && echo && sudo lvm vgs && echo && sudo lvm lvs'
alias dd='dd status=progress'
alias home='home-manager'
alias clip='xclip -sel clip'
alias suspend='systemctl suspend'
alias eject='udisksctl power-off -b'
alias nvmecheck="diff -y <(sudo nvme smart-log /dev/nvme0n1) <(sudo nvme smart-log /dev/nvme1n1)"

alias rossource='source /env/dot/.func/dev/rossource'

##APPS
alias a='atool -D -x'
alias x='xplr'
alias e=$EDITOR

alias ze='zellij'
alias pl="perles"
alias nums="dtool"
alias mcon="mconnectctl"
alias code="com.visualstudio.code"
alias hyp="hyprctl"
alias xfreerdp=xfreerdp3
alias ari='aria2c'
alias inxi='prime inxi -Fxxxz'
alias font='fc-query'
alias pomo='pomo -p /home/bresilla/.config/pomo'
alias git='hub'
alias lines='tokei'
alias search='rg --hidden'
alias vol=ncpamixer
# alias rm="gomi -rf"
alias dims="hyprctl activewindow | grep size | head -2 | tail -1 | choose 1 | tr ',' 'x'"
alias step=step-cli
alias spawn=systemd-nspawn

alias rerun='WGPU_BACKEND=vulkan WAYLAND_DISPLAY= rerun'
alias rer='WGPU_BACKEND=vulkan WAYLAND_DISPLAY= rerun --connect rerun+http://100.68.79.253:9876/proxy'

alias nixenv="nix shell --experimental-features 'nix-command flakes' github:NixOS/nixpkgs/nixos-unstable#nixos-install-tools"

##BIN
# alias bins="BIN_CONFIG=/home/bresilla/.config/bin/config.json /env/bin/bin list"
alias bino="BIN_CONFIG=/home/bresilla/.config/bin/other.json bin"

##ROBO
alias ros="ros2"

##FILES
alias own='sudo chown -R $USER:$USER'
alias exe='chmod u+x'
alias dxe='chmod u-x'

alias ali="nvim ~/.config/profile/aliases.sh"

##OTHER
alias open='nohup mimeo $1 > /dev/null 2>&1 &!'
alias rn='renamer'
alias rp='ruplacer'
alias tk='tokei'
alias mn='tldr'
alias calc='eva'
alias fq='fselect'
alias disco='diskonaut'
alias band='bandwhich'
alias tap='teip'
alias get='sudo sysget'
alias lzd='lazydocker'
alias lzg='lazygit'
alias crux='horcrux'
alias pass='gopass'
alias todo='taskwarrior-tui'
alias forge='conda'
alias hosty="sudo locdev"
alias tec="tectonic -X"
alias anytype='anytype-cli'
alias bm=bibiman
alias zik='zk --notebook-dir /doc/data/notes/zk'
alias ask=aichat
alias sks='skillshare'

# alias claude='bun x --package @anthropic-ai/claude-code claude'
# alias copilot='bun x --package @github/copilot copilot'
# alias gemini='bun x --package @google/gemini-cli gemini'
# alias openspec='bun x --package @fission-ai/openspec@latest openspec'

##MEDIA
alias color='pastel format hex' 
alias cal='cal -ymw'
alias mul="mullvad"
alias oat="oatmeal"


####         AKA
alias aka='aka -f ~/.aliases' # Added by aka
alias inc='incus' # Added by aka
alias lxc='incus' # Added by aka
