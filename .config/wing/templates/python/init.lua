...
-- Each flavour reaches Python a different way, so the tool to look for is the flavour's own.
local flavour_tool = {
  nix = nil,
  uv = "uv",
  pixi = "pixi",
...
wing.on.check(function(ctx)
  ...
  local missing = {}
  if not wing.sys.has("python3") then
    missing[#missing + 1] = "python3"
  end
  local tool = flavour_tool[ctx.flavour]
  if tool and not wing.sys.has(tool) then
    missing[#missing + 1] = tool
  end
  if ctx.flavour == "nix" then
    wing.warn("  the `nix` flavour takes every package from the flake, so it cannot work without it")
    wing.warn("  consider --flavour uv, which manages its own environment")
  end
  if #missing > 0 then
