...
-- uv owns the local .venv; every command goes through it so the interpreter is the project's.
local function run_py(...) sh.uv("run", "python", ...) end
local function run_tool(tool, ...) sh.uv("run", tool, ...) end

make.recipe{ name = "setup", desc = "create and sync the local .venv",
             run = function() sh.uv("sync") end }
