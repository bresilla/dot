...
-- Everything comes from the Nix dev shell. There is no virtual environment to create, so `setup`
-- only checks that the shell really is the one this project expects.
local function run_py(...) sh.python3(...) end
local function run_tool(tool, ...) sh[tool](...) end

make.recipe{
  name = "setup",
  desc = "check the dev shell has what this project needs",
  run = function()
    sh.python3("-c", "import pytest")
    sh.ruff("--version")
  end,
}
