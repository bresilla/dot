...
-- The name is fixed at generation time; the version comes from dune-project, the language's own
...
local VERSION = ((oslo.fs.read("dune-project") or ""):match('%(version%s+([%d%.]+)%)')) or "0.0.0"
...
-- dune is the build system; these recipes only drive it. Libraries, executables and tests are
-- declared in the dune files beside the sources.
local BIN = "_build/default/bin/main.exe"

make.recipe{
  name = "build",
  desc = "the library and the binary",
  run = function()
    sh.dune("build")
    report(BIN)
  end,
...
make.recipe{
  ...
  run = function(a)
    ...
    sh.dune(table.unpack(argv))
  end,
}
make.alias("r", "run")

make.recipe{ name = "test", desc = "the suite", run = function() sh.dune("test") end }
make.alias("t", "test")

make.recipe{ name = "check", desc = "build without running anything",
             run = function() sh.dune("build", "@check") end }
make.alias("vet", "check")

-- ocamlformat is driven through dune's own aliases rather than invoked directly: dune knows which
-- files belong to the project, and `@fmt` is the same check its CI convention uses.
make.recipe{ name = "fmt", desc = "format the sources",
             run = function() sh.dune("build", "@fmt", "--auto-promote") end }

make.recipe{ name = "fmt-check", desc = "fail if anything is unformatted",
             run = function() sh.dune("build", "@fmt") end }

make.recipe{ name = "clean", desc = "remove every build output",
             run = function() sh.dune("clean") end }
