...
make.recipe{ name = "build", desc = "the binary",
             run = function()
               sh.go("build", "-ldflags", LDFLAGS, "-o", NAME, "./src")
               report(NAME)
             end }
make.alias("b", "build")
...
make.recipe{
  ...
  run = function(a)
    ...
    sh.go(table.unpack(argv))
  end,
}
make.alias("r", "run")

make.recipe{ name = "test", desc = "the suite", run = function() sh.go("test", "./...") end }
make.alias("t", "test")

make.recipe{ name = "test-all", desc = "the suite under the race detector",
             run = function() sh.go("test", "-race", "-count=1", "./...") end }

make.recipe{
  name = "cover",
  desc = "the suite, with coverage",
  run = function()
...
make.recipe{ name = "vet", desc = "go vet", run = function() sh.go("vet", "./...") end }
make.alias("check", "vet")

make.recipe{ name = "fmt", desc = "format the tree and tidy modules",
             run = function() sh.gofmt("-w", "."); sh.go("mod", "tidy") end }

make.recipe{
  name = "fmt-check",
  desc = "fail if anything is unformatted",
  run = function()
    local listed = oslo.run{ "gofmt", "-l", ".", capture = true }
    assert(listed.ok, "gofmt failed")
    local unformatted = (listed.out or ""):gsub("%s+$", "")
    assert(unformatted == "", "gofmt needed on:\n" .. unformatted)
  end,
}

make.recipe{ name = "tidy", desc = "tidy go.mod and go.sum",
             run = function() sh.go("mod", "tidy") end }

make.recipe{ name = "clean", desc = "remove every build output",
             run = function() sh.go("clean"); sh.rm("-f", NAME, "coverage.out") end }
