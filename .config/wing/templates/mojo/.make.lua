...
-- Mojo has no manifest file to keep a version in, so the version is the latest git tag --
...
-- `mojo` is the whole toolchain: compiler, runner and formatter behind one driver. There is no
-- build system to choose. `-I src` is what lets one file in src/ import another.
local SRC = "src"
...
make.recipe{
  ...
  run = function()
    sh.mkdir("-p", "target")
    sh.mojo("build", "-I", SRC, SRC .. "/main.mojo", "-o", BIN)
    report(BIN)
  end,
}
make.alias("b", "build")
...
make.recipe{
  ...
  run = function(a)
    local argv = { "run", "-I", SRC, SRC .. "/main.mojo" }
...
make.recipe{
  name = "test",
  desc = "the suite",
  run = function() sh.mojo("run", "-I", SRC, SRC .. "/main_test.mojo") end,
}
make.alias("t", "test")

-- No check-only mode, so the check is a build whose output is thrown away.
make.recipe{
  name = "check",
  desc = "compile the sources without keeping the binary",
  run = function()
    local scratch = "/tmp/" .. NAME .. "-check"
    sh.mojo("build", "-I", SRC, SRC .. "/main.mojo", "-o", scratch)
    sh.mojo("build", "-I", SRC, SRC .. "/main_test.mojo", "-o", scratch .. "-test")
    sh.rm("-f", scratch, scratch .. "-test")
  end,
}
make.alias("vet", "check")

make.recipe{ name = "fmt", desc = "format the sources",
             run = function() sh.mojo("format", SRC) end }

-- `mojo format` has no check mode, so the sources are formatted in a copy and compared.
make.recipe{
  name = "fmt-check",
  desc = "fail if anything is unformatted",
  run = function()
    local scratch = "/tmp/" .. NAME .. "-mojofmt"
    sh.rm("-rf", scratch)
    sh.cp("-r", SRC, scratch)
    sh.mojo("format", "-q", scratch)
    local same = oslo.run{ "diff", "-r", SRC, scratch }
    sh.rm("-rf", scratch)
    assert(same.ok, "mojo format needed: run `make fmt`")
  end,
}
