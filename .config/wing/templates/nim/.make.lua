...
-- The name is fixed at generation time; the version comes from {{snake_name}}.nimble, the language's own
...
local VERSION = ((oslo.fs.read("{{snake_name}}.nimble") or ""):match('version%s*=%s*"([^"]+)"')) or "0.0.0"
