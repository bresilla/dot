...
wing.on.check(function(ctx)
  ...
  local missing = {}
  for _, tool in ipairs({ "valac", "meson", "ninja" }) do
  ...
  -- Vala emits C, so a Vala compiler on its own is not enough to produce a binary.
  if wing.sys.has("valac") and not wing.sys.has("cc") and not wing.sys.has("gcc") then
    wing.warn("  valac is here but no C compiler is; vala compiles through C and needs one")
  end
end)
