...
-- The name is fixed at generation time; the version comes from {{kebab_name}}.cabal, the language's own
...
-- Anchored to the start of a line: a .cabal opens with `cabal-version:`, which an unanchored
-- match would find first and report the format's version as the project's.
local VERSION = ((oslo.fs.read("{{kebab_name}}.cabal") or ""):match('\nversion:%s*([%d%.]+)')) or "0.0.0"
...
-- cabal is the build system; these recipes only drive it. What this package is made of -- modules,
-- dependencies, ghc options -- lives in the .cabal file.
local BIN = "dist-newstyle/" .. NAME

-- cabal puts the binary somewhere version- and compiler-specific, so it is found rather than
-- guessed. Walked with find because that path nests deeper than a single glob level.
local function built_binary()
  local found = oslo.run{ "find", "dist-newstyle", "-type", "f", "-name", NAME,
                          "-perm", "-u+x", capture = true }
  for path in (found.out or ""):gmatch("[^\n]+") do
    return path
  end
  return BIN
...
make.recipe{
  name = "build",
  desc = "the library, the binary and the tests",
  run = function()
    sh.cabal("build", "all")
    report(built_binary())
  end,
...
make.recipe{
  ...
  run = function(a)
    ...
    sh.cabal(table.unpack(argv))
  end,
}
make.alias("r", "run")

make.recipe{ name = "test", desc = "the suite",
             run = function() sh.cabal("test", "--test-show-details=direct") end }
make.alias("t", "test")

make.recipe{ name = "check", desc = "lint with hlint",
             run = function() sh.hlint("app", "src", "test") end }
make.alias("vet", "check")

-- Every Haskell source this project owns, which is what the formatter is pointed at.
local function sources()
  local found = oslo.run{ "find", "app", "src", "test", "-type", "f", "-name", "*.hs",
                          capture = true }
  local files = {}
...
make.recipe{ name = "fmt", desc = "format the sources",
             run = function() sh.ormolu("--mode", "inplace", table.unpack(sources())) end }

make.recipe{ name = "fmt-check", desc = "fail if anything is unformatted",
             run = function() sh.ormolu("--mode", "check", table.unpack(sources())) end }

make.recipe{ name = "tidy", desc = "refresh the package index",
             run = function() sh.cabal("update") end }

make.recipe{ name = "clean", desc = "remove every build output",
             run = function() sh.cabal("clean") end }
...
make.recipe{
  ...
  run = function()
    ...
    sh.install("-m", "0755", built_binary(), PREFIX .. "/bin/" .. NAME)
